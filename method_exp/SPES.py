...
if __name__ == "__main__": 
    ...
    #保存记录
    os.makedirs("./SPES_result", exist_ok=True)
    cur_time = time.strftime("%m-%d-%H-%M", time.localtime())
    json_pretty_dump(func_cold, f"./SPES_result/func_cold_{cur_time}.json")
    json_pretty_dump(func_waste, f"./SPES_result/func_waste_{cur_time}.json")
    json_pretty_dump(func_invok, f"./SPES_result/func_invok_{cur_time}.json")
    json_pretty_dump(memory_size, f"./SPES_result/memory_size_{cur_time}.json")
    ...
    plt.savefig("./SPES_result/SPES_result.png")