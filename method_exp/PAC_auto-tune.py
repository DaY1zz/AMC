...
if __name__ == "__main__": 
    ...
    for func in test_func_arrcount:
        if func_class[func].type not in (REGULAR,WARM) and func not in predictable_func_ids:    #除了Regular函数和可预测函数外的函数使用PID控制器
            PID_controller[func] = PID(1/3, 1/3, 1/3, setpoint=PID_TARGET)
            ...
            for func in (func_lst + func_corr_lst): #In case of some functions staying in the memory forever
                ...
                if func in test_func_arrcount and test_func_arrcount[func][i] > 0: #invok
                    ...
                    if func_class[func].wait_time is None or func_class[func].wait_time > 0: #A new invocation seq begins currently
                        ...
                        if SHIFT and len(func_invok_seq) % 360 == 0:    # 每6个小时更新一次可预测函数 
                            is_predicable = False
                            ...
                            if is_predicable:
                                ...
                                PID_controller[func] = PID(1/3, 1/3, 1/3, setpoint=PID_TARGET)    #不再可预测的函数使用PID控制器
                    ...
                    if _type == REGULAR:
                        ...
                        if abs(func_cold_ratio - PID_TARGET) > 0.1:  # 如果当前度量值与目标值相差较大
                            PID_controller[func].Kp += 0.05
                            PID_controller[func].Ki += 0.05
                        if np.std(func_invok_seq[func]) > 5 :  
                            PID_controller[func].Kd -= 0.02

                        # 保持系数在合理范围
                        PID_controller[func].Kp = max(min(PID_controller[func].Kp, 1.5), 0.5)
                        PID_controller[func].Ki = max(min(PID_controller[func].Ki, 1), 0.2)
                        PID_controller[func].Kd = max(min(PID_controller[func].Kd, 1), 0.1)                        
    ...
    #保存记录
    os.makedirs("./PAC_auto-tune_result", exist_ok=True)
    cur_time = time.strftime("%m-%d-%H-%M", time.localtime())
    json_pretty_dump(func_cold, f"./PAC_auto-tune_result/func_cold_{cur_time}.json")
    json_pretty_dump(func_waste, f"./PAC_auto-tune_result/func_waste_{cur_time}.json")
    json_pretty_dump(func_invok, f"./PAC_auto-tune_result/func_invok_{cur_time}.json")
    json_pretty_dump(memory_size, f"./PAC_auto-tune_result/memory_size_{cur_time}.json")
